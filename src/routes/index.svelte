...
<script lang="ts">
    ...
    const userVenues = uvms.length > 0 ? venues?.filter(v => uvms.some(uv => uv?.venueId === v.id)) : [];
    const otherVenues = userVenues.length > 0 ? venues?.filter(v => !userVenues.includes(v)) : venues;
</script>
